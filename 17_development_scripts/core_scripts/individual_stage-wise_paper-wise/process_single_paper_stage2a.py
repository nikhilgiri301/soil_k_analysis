#!/usr/bin/env python3
"""
Process Single Paper for Stage 2A: Soil K-Specific Extraction
Processes ONE specific paper through Stage 2A with real API call

NO FALLBACKS - NO SIMULATION - NO AUTO-DISCOVERY

Usage:
    python process_single_paper_stage2a.py --paper "paper_name.json" --api-key YOUR_KEY
"""
...
class SinglePaperStage2AProcessor:
    """Process a single paper through Stage 2A - NO FALLBACKS"""
    ...
    async def process_paper(self, paper_data: Dict[str, Any]) -> Dict[str, Any]:
        """Process the paper through Stage 2A with REAL API call"""
        paper_id = paper_data['paper_id']
        self.logger.info(f"Processing {paper_id} through Stage 2A")
...
{json.dumps(paper_data.get('metadata', {}), indent=2)}"""
    ...
    def validate_results(self, results: Dict[str, Any]) -> Tuple[bool, str]:
        ...
        # Minimum requirement: Must have SOME soil K content
        has_soil_k_content = False
        validation_message = ""
        
        # Check for various forms of soil K content
        if isinstance(results, dict):
            # Look for soil K specific keys
            soil_k_indicators = [
                'soil_k', 'potassium', 'k_dynamics', 'k_cycling',
                'k_availability', 'exchangeable_k', 'solution_k',
                'mineral_k', 'organic_k'
            ]
            
            # Check if any soil K indicators exist in keys or values
            result_str = json.dumps(results).lower()
            for indicator in soil_k_indicators:
                if indicator in result_str:
                    has_soil_k_content = True
                    break
        ...
        if not has_soil_k_content:
            validation_message = "No soil potassium content found in extraction"
            return False, validation_message
    ...
    def save_results(self, paper_id: str, results: Dict[str, Any], 
        ...
        output_file = self.output_dir / f"{paper_id}_2a_{timestamp}.json"
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Process Single Paper through Stage 2A - NO FALLBACKS",
    ...
    processor = SinglePaperStage2AProcessor(args.api_key)
