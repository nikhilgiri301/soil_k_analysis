#!/usr/bin/env python3
"""
Master Stage 1A Processor
Orchestrates processing of all papers through Stage 1A by calling single paper processor
...
Usage:
    python master_stage1a_processor.py --api-key YOUR_KEY
    python master_stage1a_processor.py --api-key YOUR_KEY --papers paper1.json paper2.json
    python master_stage1a_processor.py --api-key YOUR_KEY --limit 5
    python master_stage1a_processor.py --api-key YOUR_KEY --force-reprocess
"""
...
class MasterStage1AProcessor:
    ...
    def check_existing_results(self, paper_name: str) -> bool:
        """Check if Stage 1A results already exist in APPROVED_RESULTS folder only"""
        approved_dir = Path("8_stage_outputs/stage_1a/approved_results")
    ...
    async def process_single_paper(self, paper_name: str) -> Dict[str, Any]:
        ...
        cmd = [
            ...
            "process_single_paper_stage1a.py",
            "--paper", paper_name,
    ...
    async def process_all_papers(self, papers: List[str]):
        """Process all papers through Stage 1A in parallel"""
        print(f"\n🚀 Starting Stage 1A processing for {len(papers)} papers")
        ...
        # Print summary
        print(f"\n{'='*80}")
        print(f"📊 STAGE 1A PROCESSING COMPLETE")
        print(f"{'='*80}")
    ...
    def save_summary(self, successful: int, failed: int, total_time: float):
        ...
        logs_dir = Path("11_validation_logs/stage_1a_processing")
        logs_dir.mkdir(parents=True, exist_ok=True)
        
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        summary_file = logs_dir / f"stage_1a_master_summary_{timestamp}.json"
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Master Stage 1A Processor - Orchestrates processing all papers",
    ...
    # Initialize master processor
    master = MasterStage1AProcessor(args.api_key)
