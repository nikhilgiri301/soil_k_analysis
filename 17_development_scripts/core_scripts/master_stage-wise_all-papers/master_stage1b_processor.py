#!/usr/bin/env python3
"""
Master Stage 1B Processor
Orchestrates processing of all papers through Stage 1B by calling single paper processor
...
Usage:
    python master_stage1b_processor.py --api-key YOUR_KEY
    python master_stage1b_processor.py --api-key YOUR_KEY --papers paper1.json paper2.json
    python master_stage1b_processor.py --api-key YOUR_KEY --limit 5
    python master_stage1b_processor.py --api-key YOUR_KEY --force-reprocess
"""
...
class MasterStage1BProcessor:
    ...
    def check_existing_results(self, paper_name: str) -> bool:
        """Check if Stage 1B results already exist in APPROVED_RESULTS folder only"""
        approved_dir = Path("8_stage_outputs/stage_1b/approved_results")
        
        # Look for any existing result file for this paper in approved_results ONLY
        pattern = f"{paper_name}_1b_*.json"
    ...
    def check_stage1a_dependency(self, paper_name: str) -> bool:
        """Check if Stage 1A dependency exists for this paper"""
    ...
    def filter_unprocessed_papers(self, papers: List[str], force_reprocess: bool = False) -> List[str]:
        ...
        for paper in papers:
            ...
            elif not self.check_stage1a_dependency(paper):
        ...
        if missing_dependencies:
            print(f"⚠️  Found {len(missing_dependencies)} papers missing Stage 1A dependency:")
            for paper in missing_dependencies:
                print(f"   - {paper}")
            print(f"💡 Run Stage 1A first for these papers")
    ...
    async def process_single_paper(self, paper_name: str) -> Dict[str, Any]:
        ...
        cmd = [
            ...
            "process_single_paper_stage1b.py",
            "--paper", paper_name,
        ...
        try:
            ...
            print(f"⏰ {paper_name}: TIMEOUT after 10 minutes")
            try:
                process.kill()
                await process.wait()
            except:
    ...
    async def process_all_papers(self, papers: List[str]):
        """Process all papers through Stage 1B in parallel"""
        print(f"\n🚀 Starting Stage 1B processing for {len(papers)} papers")
        ...
        # Print summary
        print(f"\n{'='*80}")
        print(f"📊 STAGE 1B PROCESSING COMPLETE")
        print(f"{'='*80}")
    ...
    def save_summary(self, successful: int, failed: int, total_time: float):
        ...
        logs_dir = Path("11_validation_logs/stage_1b_processing")
        logs_dir.mkdir(parents=True, exist_ok=True)
        
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        summary_file = logs_dir / f"stage_1b_master_summary_{timestamp}.json"
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Master Stage 1B Processor - Orchestrates processing all papers",
    ...
    # Initialize master processor
    master = MasterStage1BProcessor(args.api_key)
