#!/usr/bin/env python3
"""
Master Stage 4A Processor
Orchestrates processing of all papers through Stage 4A by calling single paper processor
...
Only processes papers that have Stage 3B dependencies in approved_results

Usage:
    python master_stage4a_processor.py --api-key YOUR_KEY
    python master_stage4a_processor.py --api-key YOUR_KEY --papers paper1.json paper2.json
    python master_stage4a_processor.py --api-key YOUR_KEY --limit 5
    python master_stage4a_processor.py --api-key YOUR_KEY --force-reprocess
"""
...
class MasterStage4AProcessor:
    ...
    def check_existing_results(self, paper_name: str) -> bool:
        """Check if Stage 4A results already exist in APPROVED_RESULTS folder only"""
        approved_dir = Path("8_stage_outputs/stage_4a/approved_results")
        
        if not approved_dir.exists():
            return False
        
        # Look for any Stage 4A file for this paper
        for result_file in approved_dir.glob(f"{paper_name}_4a_*.json"):
            return True
        
        return False
    
    def check_stage3b_dependency(self, paper_name: str) -> bool:
        """Check if Stage 3B dependency exists for this paper"""
        stage3b_approved_dir = Path("8_stage_outputs/stage_3b/approved_results")
        
        if not stage3b_approved_dir.exists():
            return False
        
        # Look for Stage 3B file for this paper
        for stage3b_file in stage3b_approved_dir.glob(f"{paper_name}_3b_*.json"):
            return True
    ...
    def filter_papers(self, papers: List[str], force_reprocess: bool = False) -> List[str]:
        ...
        for paper in papers:
            # Check if Stage 3B dependency exists
            if not self.check_stage3b_dependency(paper):
        ...
        if missing_dependencies:
            print(f"⚠️  Found {len(missing_dependencies)} papers missing Stage 3B dependencies:")
    ...
    async def process_single_paper(self, paper_name: str) -> Dict[str, Any]:
        ...
        cmd = [
            ...
            "process_single_paper_stage4a.py",
            "--paper", paper_name,
    ...
    async def process_all_papers(self, papers: List[str]):
        """Process all papers through Stage 4A in parallel"""
        if not papers:
            print("No papers to process.")
            return
        
        print(f"\n📊 Filtered {len(papers)} papers to process")
        print(f"\n🚀 Starting Stage 4A processing for {len(papers)} papers")
        ...
        # Summary
        print(f"\n{'='*80}")
        print(f"📊 STAGE 4A PROCESSING COMPLETE")
        print(f"{'='*80}")
    ...
    def save_summary(self):
        ...
        log_dir = Path("11_validation_logs/stage_4a_processing")
        log_dir.mkdir(parents=True, exist_ok=True)
        
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        summary_file = log_dir / f"stage_4a_master_summary_{timestamp}.json"
...
async def main():
    parser = argparse.ArgumentParser(description='Master Stage 4A processor for all papers')
    ...
    processor = MasterStage4AProcessor(args.api_key)
