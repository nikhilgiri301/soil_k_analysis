#!/usr/bin/env python3
"""
Master Stage 4B Processor
Orchestrates processing of all papers through Stage 4B by calling single paper processor
...
Only processes papers that have Stage 4A dependencies in approved_results

Usage:
    python master_stage4b_processor.py --api-key YOUR_KEY
    python master_stage4b_processor.py --api-key YOUR_KEY --papers paper1.json paper2.json
    python master_stage4b_processor.py --api-key YOUR_KEY --limit 5
    python master_stage4b_processor.py --api-key YOUR_KEY --force-reprocess
"""
...
class MasterStage4BProcessor:
    ...
    def check_existing_results(self, paper_name: str) -> bool:
        """Check if Stage 4B results already exist in APPROVED_RESULTS folder only"""
        approved_dir = Path("8_stage_outputs/stage_4b/approved_results")
        
        if not approved_dir.exists():
            return False
        
        # Look for any Stage 4B file for this paper
        for result_file in approved_dir.glob(f"{paper_name}_4b_*.json"):
            return True
        
        return False
    
    def check_stage4a_dependency(self, paper_name: str) -> bool:
        """Check if Stage 4A dependency exists for this paper"""
        stage4a_approved_dir = Path("8_stage_outputs/stage_4a/approved_results")
        
        if not stage4a_approved_dir.exists():
            return False
        
        # Look for Stage 4A file for this paper
        for stage4a_file in stage4a_approved_dir.glob(f"{paper_name}_4a_*.json"):
            return True
    ...
    def filter_papers(self, papers: List[str], force_reprocess: bool = False) -> List[str]:
        ...
        for paper in papers:
            # Check if Stage 4A dependency exists
            if not self.check_stage4a_dependency(paper):
        ...
        if missing_dependencies:
            print(f"⚠️  Found {len(missing_dependencies)} papers missing Stage 4A dependencies:")
    ...
    async def process_single_paper(self, paper_name: str) -> Dict[str, Any]:
        ...
        cmd = [
            ...
            "process_single_paper_stage4b.py",
            "--paper", paper_name,
    ...
    async def process_all_papers(self, papers: List[str]):
        """Process all papers through Stage 4B in parallel"""
        if not papers:
            print("No papers to process.")
            return
        
        print(f"\n📊 Filtered {len(papers)} papers to process")
        print(f"\n🚀 Starting Stage 4B processing for {len(papers)} papers")
        ...
        # Summary
        print(f"\n{'='*80}")
        print(f"📊 STAGE 4B PROCESSING COMPLETE")
        print(f"{'='*80}")
    ...
    def save_summary(self):
        ...
        log_dir = Path("11_validation_logs/stage_4b_processing")
        log_dir.mkdir(parents=True, exist_ok=True)
        
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        summary_file = log_dir / f"stage_4b_master_summary_{timestamp}.json"
...
async def main():
    parser = argparse.ArgumentParser(description='Master Stage 4B processor for all papers')
    ...
    processor = MasterStage4BProcessor(args.api_key)
