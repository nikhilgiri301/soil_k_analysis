...
class StageTestFramework:
    ...
    def validate_stage_output(self, results: Dict[str, Any], expected_stage: str = None) -> Tuple[bool, List[str]]:
        ...
        # Stage-specific validation with flexible, real-world checks
        if stage_to_validate in ['1a', '2a']:  # Extraction stages
            if stage_to_validate == '1a':
                # Stage 1A flexible validation - adapted from Stage 2A working logic
                ...
                # Require at least metadata and some form of content
                if not has_metadata:
                ...
                # Stage 2A flexible validation - look for any soil K specific content
                has_soil_k = any(key in results for key in ['soil_k_specific_analysis', 'k_cycling_mechanisms', 'potassium_dynamics', 'quantitative_soil_k_data'])
                has_metadata = 'paper_metadata' in results
                
                if not has_metadata:
                    issues.append("Missing paper_metadata section")
                if not has_soil_k:
                    issues.append("Missing soil K-specific content")
                    
        elif stage_to_validate in ['1b', '2b', '3b', '4b']:  # Validation stages
            # Flexible validation for validation stages - look for success indicators
            has_success = any(key in results for key in ['success', 'validation_quality', 'validation_certification', 'validation_result'])
            if not has_success:
                issues.append("Missing validation result indicators")
                
        elif stage_to_validate == '3a':  # Synthesis stage
            # Check for key synthesis sections flexibly
            has_authority = 'paper_authority' in results
            has_findings = any(key in results for key in ['integrated_soil_k_findings', 'synthesis_findings', 'key_insights'])
            
            if not has_authority:
                issues.append("Missing paper_authority section")
            if not has_findings:
                issues.append("Missing synthesis findings section")
                
        elif stage_to_validate == '4a':  # Client mapping stage
            # Flexible client mapping validation
            has_identification = any(key in results for key in ['paper_identification', 'paper_info', 'metadata'])
            has_mapping = any(key in results for key in ['direct_evidence_mapping', 'client_mapping', 'evidence_map'])
            
            if not has_identification:
                issues.append("Missing paper identification section")
            if not has_mapping:
                issues.append("Missing evidence mapping section")
